...
const uint8_t lcd6100_font_large::m_font_table[FONT_LARGE_NR_CHARS][FONT_LARGE_BYTES_PER_CHAR] = {
  ...
  {0x00,0x70,0xD8,0xD8,0x70,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}  // DEL  0x7f
};
...
lcd6100_font_large::lcd6100_font_large(void)
{
  m_height = 16;
  m_width  =  8;
  m_bytes_per_char = FONT_LARGE_BYTES_PER_CHAR;
}
...
const uint8_t* lcd6100_font_large::get_font_table(char c)
{
  if ( (c >= 0x20) && (c <= 0x7f) ) {
    return &m_font_table[c - 0x20][0];
  }
  else {
    return 0; // Character not in table
  }
}
