			 ...
			 loff_t *f_pos)
{
  ...
  printk(KERN_ALERT "BACH: bach_read, ID=%d, count=%d\n",
	 dev->id, count);
			  ...
			  loff_t *f_pos)
{
  ...
  printk(KERN_ALERT "BACH: bach_write, ID=%d, count=%d\n",
	 dev->id, count);
		     ...
		     struct file *filp)
{
  ...
  printk(KERN_ALERT "BACH: bach_open, ID=%d\n", dev->id);
			...
			struct file *filp)
{
  ...
  printk(KERN_ALERT "BACH: bach_release, ID=%d\n", dev->id);
