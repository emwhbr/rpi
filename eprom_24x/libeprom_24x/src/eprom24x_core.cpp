...
* Name get_last_error
*
* Description Returns the error information held by LIBEPROM24x.
*
* Parameters status  IN/OUT  Pointer to a buffer to hold error information.
*
* Error handling Returns EPROM24x_SUCCESS if successful
*                otherwise EPROM24x_MUTEX_FAILURE
*
****************************************************************************/
...
long eprom24x_core::set_error(eprom24x_exception rxp)
{
  ...
  for (unsigned i=0; i < frames.active_frames; i++) {
    sprintf(buffer, "0x%016x", frames.frames[i]);
